...
Module Type INSTR (UT:UTYPE) (T:TYPE UT) (Var:VAR UT T) (Proc:PROC UT T)
 ...
 | Random : forall t, Var.var t -> DE.support t -> baseInstr
 | IAssert : E.expr T.Bool -> baseInstr.
 ...
 Notation Local "'Assert' e" := (Instr (IAssert e)) (at level 65).
...
Module MakeInstr (UT:UTYPE) (T:TYPE UT) (Var:VAR UT T) (Proc:PROC UT T)
 (Uop:UOP UT T) (O:OP UT T Uop) (Mem:MEM UT T Var) 
 (E:EXPR UT T Var Mem Uop O) (US:USUPPORT UT T Var Mem Uop O E)
 (DE:DEXPR UT T Var Mem Uop O E US) <: INSTR UT T Var Proc Uop O Mem E US DE.
 ...
 Definition bi_eqb (x y:baseInstr) : bool :=
  ...
  | Random t1 v1 s1, Random t2 v2 s2 =>
    if Var.veqb v1 v2 then DE.seqb s1 s2 else false
  | IAssert e1, IAssert e2 => E.eqb e1 e2 
  | _, _ => false
  end.
 ...
 Proof.
  ...
  inversion H; clear H; subst.
  rewrite (T.inj_pair2 H3); clear H2 H3.
  generalize (DE.seqb_spec s s0); destruct (DE.seqb s s0); intros; subst; trivial.
  intros Heq; apply H; inversion Heq.  
  apply (T.inj_pair2 H1).
  intros Heq; apply H; inversion Heq; trivial. 
  subst; rewrite (T.inj_pair2 H2); constructor. 
  generalize (E.eqb_spec e e0); destruct (E.eqb e e0); intros; subst; trivial.
  intros Heq; apply H; injection Heq; intros; trivial.
 Qed.
