...
Module Make (SemO:SEM_OPT).
 ...
 Section CONTEXT.
  ...
  Definition context_bi (i:I.baseInstr) :=
   ...
   | I.IAssert e    => Vset.disjoint (fv_expr e) nR
   end.
...
Section CONTEXT_PROOF.
  ...
  Proof.
   ...
   unfold notR, EP1 in *; intro; apply H2.
   rewrite (@EqObs_e_fv_expr _ e _ m2 
    (m2 {!v <-- D_points (DE.discrete_support s m2) i!})); trivial.
   apply req_mem_upd_disjoint.
   apply not_true_is_false; rewrite VsetP.add_spec, VsetP.union_spec in H; tauto.

   eapply equiv_sub; [ | | apply equiv_assert with (P:=Meq /-\ ~-EP1 e)].
  ...
  Proof.
   ...
   unfold context_bi in c.
   destruct i; [destruct c as (H1, H2) | destruct c as (H1, H2) | ].
   ...
   assert (equiv (Meq /-\ EP1 e) E1 [v <$- s] E2 [v <$- s] (Meq /-\ EP1 e)).
    ...
    unfold EP1; rewrite <- (@EqObs_e_fv_expr _ e _ m2 
     (m2 {!v <-- D_points (DE.discrete_support s m2) i!})); trivial.
    apply req_mem_upd_disjoint.
    apply not_true_is_false; rewrite VsetP.add_spec, VsetP.union_spec in H1; tauto.
   rewrite deno_cons; symmetry; rewrite deno_cons; symmetry.
   ...
   refine (equiv_deno H0 _ _ _ _ _ _); unfold Meq, EP1.
    ...
    transitivity (([[ [Assert e0; y <$- d] ]]) E2 m).
    apply eq_distr_intro; intro f.
    rewrite deno_cons_elim; symmetry; rewrite deno_cons_elim; symmetry. 
    rewrite Mlet_simpl, Mlet_simpl.
    eapply equiv_deno.
    apply equiv_assert with (P:=Meq /-\ EP1 e).
    intros m1 m2 [ [? ?] [? ?] ].
    rewrite deno_cond_elim.
    unfold EP1 in H1; rewrite <-H0, H1.
    repeat rewrite deno_random_elim; trivial.
    repeat split; trivial.
    
    apply eq_distr_intro; intro f.
    eapply equiv_deno; 
     [eapply equiv_swap with 
      (c1:=[Assert e0]) (c2:=[y <$- d]) 
      (I1:=fv_expr e0) (O1:=Vset.empty) (O2:=Vset.singleton y) | | ]; trivial.
    apply Modify_assert.
    apply Modify_random.
    eapply equiv_sub; [ | | apply equiv_assert with (P:=kreq_mem (fv_expr e0))].
    intros; split; trivial.
    apply depend_only_fv_expr; trivial.
    intros; apply req_mem_empty.
    apply EqObs_random.
    apply VsetP.disjoint_sym; trivial.
   ...
   transitivity (mu (DE.eval_support d m0) (fun _  => 0)).
   rewrite mu_0; trivial.
   apply (mu_stable_eq (DE.eval_support d m0)).
   ...
   rewrite Mcomp.
   replace (DE.eval_support d (return_mem E1 x f m m0)) with 
    (DE.eval_support d m0).
   apply Mlet_eq_compat; trivial.
   ...
   replace (DE.eval_support d (init_mem E1 f args m)) with 
       (DE.eval_support d m).
   apply Mlet_eq_compat; trivial.
 ...
 Lemma equiv_context_true : forall (E1 E2:env) (C1 C2:cmd) (t0:T.type) 
  (y:Var.var t0) (d:DE.support t0) (e:E.expr T.Bool) c1 c2,
