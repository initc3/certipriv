...
Module Make (SemO:SEM_OPT).
 ...
 Module LS2 := LazySampling.Make SemO.
 Export LS2.
 ...
 | Feq_assoc_except : forall t1 t2, 
   Var.var t1 -> Var.var (T.List (T.Pair t1 t2)) -> formula
 | Fforall_random : forall t, DE.support t -> Var.var t -> formula -> formula.
 ...
 Section INTERP.
  ...
  Fixpoint interp_F (F:formula) : mem_rel:=
   match F with
    ...
    | Fforall_random t d x F =>
      fun k m1 m2 => forall v, interp_F F (m1{!x <-- v!}) (m2{!x <-- v!})
   end.
  ...
  Lemma eqobsrel_tail_rm1_spec : forall (P : mem_rel) (E1 : env) 
   (r1 r1': cmd) (E2 : env) (c2 : cmd) (O : Vset.t) Q Q',
  ...
  Proof.
   ...
   unfold req_mem_rel,andR, kreq_mem; simpl; 
  ...
  Section EOR_tail_cmd.
   ...
   Hypothesis Hrec_i : forall E1 i1 E2 i2 O Q O' Q',
    eqobsrel_tail_rev_i i1 i2 O Q = Some (O', Q') ->
    equiv (req_mem_rel O' (interp_F Q'))
    E1 [i1] E2 [i2] (req_mem_rel O (interp_F Q)).
   ...
   Lemma eqobsrel_tail_rev_aux_spec : forall n P E1 r1 E2 r2 O Q c1 c2 O' Q',
   ...
   Proof.
    ...
    rewrite Heq in H; destruct r1'; 
     try (injection H; clear H; intros; subst; trivial).
   ...
   Definition eqobsrel_tail_rev_i_body (i1 i2:I.instr) O Q := 
    match i1 with 
     ...
     | I.Instr (I.IAssert e1) =>
       match i2 with
       | I.Instr (I.IAssert e2) =>
         if E.eqb e1 e2 then
          let O' := fv_expr_extend e1 O in
          let Q' := Fand Q (Fbool1 e1) in
            Some (O', Q') 
         else None
       | _ => None
       end
   ...
   Proof.
    ...
    destruct b; try discriminate.
    generalize (Var.eqb_spec v v0); destruct (Var.eqb v v0); 
     intros; try discriminate.
    inversion H0; clear H0; subst.
    assert (W:= T.inj_pair2 H3); clear H3; subst.
    generalize (E.eqb_spec e e0); destruct (E.eqb e e0); intros;
    ...
    inversion H0; clear H0; subst.
    assert (W:= T.inj_pair2 H3); clear H3; subst.
    generalize (DE.seqb_spec s s0); destruct (DE.seqb s s0); 
    ...
    destruct i2; try discriminate.
    destruct b; try discriminate.
    generalize (E.eqb_spec e e0); destruct (E.eqb e e0); 
     intros; subst; try discriminate.
    injection H; intros; subst.
    eapply equiv_sub; 
     [ | | apply equiv_assert with (P:=req_mem_rel O (interp_F Q))].
    intros k m1 m2 [? ?]; repeat split.
    apply req_mem_weaken with (2:=H0).
    rewrite union_fv_expr_spec; auto with set.
    simpl in H1; destruct H1; trivial.
    apply depend_only_fv_expr.
    apply req_mem_weaken with (2:=H0).
    rewrite union_fv_expr_spec; auto with set.
    intros k m1 m2 [ [? ?] ?]; repeat split; trivial.
  ...
  Lemma eqobsrel_tail_rev_i_spec : forall n E1 i1 E2 i2 O Q O' Q',
   ...
   equiv (req_mem_rel O' (interp_F Q')) E1 [i1] E2 [i2] 
   (req_mem_rel O (interp_F Q)).
  Proof.
  ...
  Section ABS_INTERP.
   ...
   Section ABS_EVAL.
    ...
    Fixpoint interp_av (t:T.type) (v:abs_val t) :=
     match v in abs_val t0 return T.interp k t0 with
      ...
      | Afind t f l =>
        find_default (fun v => interp_av (f v)) (interp_av l) (T.default k t)
     end.
    ...
    Fixpoint abs_eval (t:T.type) (e:E.expr t) (m:abs_mem) {struct e} : abs_val t :=
     match e in (E.expr t0) return abs_val t0 with
      ...
      | E.Eop op la =>
        Aop op (dmap abs_val (fun t0 (e0:E.expr t0) => abs_eval e0 m) la)
    ...
    Lemma abs_get_correct : forall m tx (x:Var.var tx),
    ...
    Proof.
     induction e using E.expr_ind2 with
      (Pl := fun lt la => forall m,
        ...
        dmap (T.interp k)
        (fun t0 (e0 : E.expr t0) => E.eval_expr e0 (interp_mem m)) la); 
     ...
     rewrite IHi, IHe1; trivial.
     rewrite IHe2; generalize (E.eval_expr e2 (interp_mem m)); 
      intros i; unfold find_default.
     replace (find (fun v0 => interp_av (abs_eval e1 (AMupd m v (Aval t v0)))) i)
    ...
    | ADuser : forall t, US.usupport t -> abs_support t
    | ADprod : forall t1 t2, 
      abs_support t1 -> abs_support t2 -> abs_support (T.Pair t1 t2).

    Fixpoint abs_eval_support t (s:DE.support t) m :=
     match s in DE.support t0 return abs_support t0 with 
      | DE.Dbool => ADbool  
      | DE.Dnat e => ADnat (abs_eval e m)
      | DE.DZ e1 e2 => ADZ (abs_eval e1 m) (abs_eval e2 m)
      | DE.Duser t s => ADuser s
      | DE.Dprod t1 t2 s1 s2 => 
       ADprod (abs_eval_support s1 m) (abs_eval_support s2 m)
     end.

    Fixpoint interp_asup t (s:abs_support t) :=
     match s in abs_support t0 return (Distr (T.interp k t0)) with
      | ADbool => Flip 
      | ADnat e => Random (interp_av e)
      | ADZ e1 e2 => RandomZ (interp_av e1) (interp_av e2)
      | ADuser t s => 
        if US.fv_distr s [=?] Vset.empty then US.eval s init else distr0 _
      | ADprod t1 t2 s1 s2 => prod_distr (interp_asup s1) (interp_asup s2)
     end.

    Fixpoint valid_asup t (s:DE.support t) :=
     match s with
     | DE.Duser t us => US.fv_distr us [=?] Vset.empty
     | DE.Dprod t1 t2 s1 s2 => valid_asup s1 && valid_asup s2
     | _ => true
     end.

    Lemma abs_eval_support_correct : forall t (s:DE.support t) m,
     valid_asup s ->
     interp_asup (abs_eval_support s m) = DE.eval_support s (interp_mem m).
    Proof.
     induction s; simpl; trivial; intros.
     rewrite abs_eval_correct; trivial.
     repeat rewrite abs_eval_correct; trivial.
     rewrite H; simpl.
     refine (US.depend_only_fv_distr _).
     rewrite H; apply req_mem_empty.
   ...
   Inductive abs_formula : Type :=
    ...
    | AFeq_assoc_except : forall t1 t2, 
      abs_val t1 -> 
      abs_val (T.List (T.Pair t1 t2)) -> 
      abs_val (T.List (T.Pair t1 t2)) -> abs_formula
    | AFforall_random :
   ...
   Fixpoint interp_aF (F:abs_formula) :=
    ...
    | AFforall_random t d f => 
       let s := interp_asup init1 d in
       forall v, interp_aF (f v)
    end.
   ...
   Fixpoint abs_eval_F (F:formula) (m1 m2:abs_mem) : abs_formula :=
    ...
    | Fupd_p F t x e => 
      abs_eval_F F (AMupd m1 x (abs_eval e m1)) (AMupd m2 x (abs_eval e m2))
   ...
   Proof.
    ...
    split; intros.   
    assert (W:=H v0); rewrite IHF in W; trivial.
    rewrite IHF; simpl; trivial.
   Qed.
  ...
  Lemma eqobsrel_tail_n_correct : forall n P E1 c1 E2 c2 O Q c1' c2' O' Q',
   eqobsrel_tail_n n c1 c2 O Q = Quad c1' c2' O' Q' ->
   equiv P E1 c1' E2 c2' 
   (req_mem_rel O' (fun k m1 m2 => interp_aF m1 m2 (Q' k))) ->
   equiv P E1 c1 E2 c2 (req_mem_rel O (interp_F Q)).
  Proof.
   unfold eqobsrel_tail_n, eqobsrel_tail_rev_i100; intros.
   rewrite <- (rev_involutive c1), <- (rev_involutive c2).
   case_eq (eqobsrel_tail_rev_aux 
    (eqobsrel_tail_rev_i 100) n (rev c1) (rev c2) O Q); 
   intros c1'' c2'' O'' Q'' Heq;
