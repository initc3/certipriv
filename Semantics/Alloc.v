...
Module Make (SemO:SEM_OPT).
 ...
 Definition alloc_b (i:I.baseInstr) (dep:bool) : cmd :=
  ...
  | I.Random tx x di => 
    if andb (negb dep) (Var.veqb x r) then 
      ...
      | right _ => (* unreachable *) [x <$- di]
      end 
    else [x <$- di]
  | I.IAssert e => [Assert e]    
  end.
 ...
 Definition depend_b ib dep :=
  ...
  | I.IAssert e =>
    if Vset.mem d (fv_expr e) then true else dep    
  end.
 ...
 Fixpoint alloc_i (i:I.instr) (dep:bool) {struct i} : bool * cmd :=
  ...
  Proof.
   ...
   apply equiv_call with 
    ...
    (Qf:=fun k m1 m2 => 
     eeq_mem (Vset.add d 
      (Vset.diff (Vset.union L (pi_mod p)) (Vset.union ls (pi_output p)))) m1 m2 /\
   ...
   split.
   red; intros.
   rewrite VsetP.add_spec, VsetP.diff_spec, 
    VsetP.union_spec, VsetP.union_spec in H7.
  ...
  Proof.
   ...
   (* Assert *)
   case_eq (Vset.mem d (fv_expr e)); intro; destruct dep; simpl.
   apply equiv_eq_sem; trivial.
   eapply equiv_weaken; [ | apply equiv_eq_sem; trivial].
   intros k m1 m2 Heq; rewrite Heq; apply eeq_mem_refl.
   apply equiv_eq_sem; trivial.
   eapply equiv_sub; 
    [ | | apply equiv_assert with (P:=eeq_mem (Vset.singleton d)); trivial].
   intros k m1 m2; repeat split; trivial.
   apply not_fv_eeq_mem with (1:=H); trivial.
