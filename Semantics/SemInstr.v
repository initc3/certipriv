...
Module Type SEM_INSTR (UT:UTYPE) (T:TYPE UT) (Var:VAR UT T) (Proc:PROC UT T)
 (Uop:UOP UT T) (O:OP UT T Uop) (Mem:MEM UT T Var) 
 (E:EXPR UT T Var Mem Uop O) (US:USUPPORT UT T Var Mem Uop O E)
 (DE:DEXPR UT T Var Mem Uop O E US)
 (I:INSTR UT T Var Proc Uop O Mem E US DE).
 ...
 Notation "'Assert' e" := (I.Instr (I.IAssert e)) (at level 65).
 ...
 Parameter sample_lossless : forall k t (d:DE.support t) (m:Mem.t k),
  mu (@DE.eval_support k t d m) (fun _ => 1) == 1.
 ...
 Section DENO_FACTS.   
  ...
  Parameter cdeno_random_elim : forall t (x:Var.var t) s (mn:Mem.t k * nat) f,
   mu ([[[ [x <$- s] ]]] E mn) f ==
   let (s,n) := DE.ceval_support s (fst mn) in
    mu s (fun v => f (fst mn {!x <-- v!}, snd mn + n))%nat.
  
  Parameter cdeno_random : forall t (x:Var.var t) s (mn:Mem.t k * nat),
   [[[ [x <$- s] ]]] E mn ==
   let (s,n) := DE.ceval_support s (fst mn) in
    Mlet s (fun v => Munit (fst mn {!x <-- v!}, snd mn + n))%nat.
 
  Parameter deno_random_elim : forall t (x:Var.var t) s (m:Mem.t k) f,
   mu ([[ [x <$- s] ]] E m) f== 
   mu (DE.eval_support s m) (fun v => f (m{!x <-- v!})).
  
  Parameter deno_random : forall t (x:Var.var t) s (m:Mem.t k),
   [[ [x <$- s] ]] E m == 
   Mlet (DE.eval_support s m) (fun v => Munit (m{!x <-- v!})).
 
  Parameter cdeno_assert_elim : forall e (mn:Mem.t k * nat) f,
   mu ([[[ [Assert e] ]]] E mn) f ==
   if E.eval_expr e (fst mn) then f mn else 0.
  
  Parameter cdeno_assert : forall e (mn:Mem.t k * nat),
   [[[ [Assert e] ]]] E mn ==
   if E.eval_expr e (fst mn) then Munit mn else distr0 _.

  Parameter deno_assert_elim : forall e (m:Mem.t k) f,
    mu ([[ [Assert e] ]] E m) f == if E.eval_expr e m then f m else 0.
  
  Parameter deno_assert : forall e (m:Mem.t k),
   [[ [Assert e] ]] E m == if E.eval_expr e m then Munit m else distr0 _.
...
Module Make (UT:UTYPE) (T:TYPE UT) (Var:VAR UT T) (Proc:PROC UT T)
 (Uop:UOP UT T) (O:OP UT T Uop) (Mem:MEM UT T Var) 
 (E:EXPR UT T Var Mem Uop O) (US:USUPPORT UT T Var Mem Uop O E)
 (DE:DEXPR UT T Var Mem Uop O E US) (I:INSTR UT T Var Proc Uop O Mem E US DE) 
 <: SEM_INSTR UT T Var Proc Uop O Mem E US DE I.
 ...
 Section K.
  ...
  Definition base_step i (mn:Mem.t k * nat) : Distr (Mem.t k * nat) :=
   let (m, n) := mn in
    ...
    | I.Random t x d =>
      let sn := @DE.ceval_support k t d m in
      Mlet (fst sn) (fun v => Munit (Mem.upd m x v, (n+snd sn)%nat))
    | I.IAssert e =>
      let v := E.eval_expr e m in
      if v then Munit mn else distr0 _
    end.
  ...
  Ltac ltrivial :=
   ...
   match goal with 
    ...
    | |- context [DE.ceval_support ?S ?M] => destruct (DE.ceval_support S M)
    | _ => idtac
   end;
   trivial.
  ...
  Proof.
   ...
   simpl; case (E.eval_expr e m); simpl; trivial.
   rewrite plus_comm; trivial. 
  Qed.

  Lemma sample_lossless : forall k t (s:DE.support t) (m:Mem.t k),
   mu (@DE.eval_support k t s m) (fun _ => 1) == 1.
  Proof.
   intros; apply DE.lossless_support.
  Qed.

  Definition is_assert i :=
   match i with
    | I.IAssert _ => true
    | _ => false
   end.

  Lemma base_step_lossless : forall i m,
   is_assert i = false ->
   mu (base_step i m) (fun _ => 1) == 1.
  Proof.
   destruct m as (m,n); destruct i; intros; simpl.   
   trivial.
   setoid_rewrite <- (DE.ceval_support_spec s m).   
   refine (sample_lossless _ _).
   discriminate.
  Qed.

  Definition carT k t : T.interp k t -> MF (T.interp k t) := 
   fun (x y:T.interp k t) => if T.i_eqb k t x y then 1 else 0.
  
  Lemma carT_cover : forall k t x, cover (eq x) (carT k t x).
  Proof.
   unfold carT, cover; intros.
   generalize (T.i_eqb_spec k0 t x x0);  case (T.i_eqb k0 t x x0); intuition.  
  Qed.
  
  Lemma base_step_comm : forall (A:Type) (d:Distr A) b m,
   prod_comm (base_step b m) d.
  Proof.
   intros A d b mn; destruct b; destruct mn as (m,n).
   apply prod_comm_Munit.
   apply prod_comm_Mlet; auto using prod_comm_Munit.
   rewrite <-DE.ceval_support_spec.  
   apply is_Discrete_commute with (1:=carT_cover k t).
   apply DE.discrete_support.
   
   simpl; case (E.eval_expr e m).
   apply prod_comm_Munit.
  ...
  Section DENO.
   ...
   Definition is_head_assert s :=
    match st_pc s with
    | I.Instr i :: _ => is_assert i
    | _ => false
    end.

   Lemma step_lossless : forall s,
    is_head_assert (fst s) = false ->
    mu (step s) (fun _ => 1) == 1.
   Proof.
    unfold step, is_head_assert; simpl; intros (s,n); simpl.
    destruct (st_pc s).
    destruct (st_stack s); trivial.
    destruct i; trivial.
    simpl; unfold star,unit; simpl.
    intro; rewrite base_step_lossless; trivial.
   Qed.
  ...
  Section DENO_FACTS. 
   ...
   Proof.
    intros t x s (m,n) f.
    rewrite cdeno_base_elim.
    simpl; case (DE.ceval_support s m); trivial.
   Qed.
   ...
   Proof.
    intros t x s m f.
    rewrite deno_base_elim, DE.ceval_support_spec; trivial.
   Qed.
   ...
   Lemma cdeno_assert_elim : forall e (mn:Mem.t k * nat) f,
    mu ([[[ [Assert e] ]]] E mn) f ==
    if E.eval_expr e (fst mn) then f mn else 0.
   Proof.
    intros e (m,n) f.
    rewrite cdeno_base_elim; simpl.
    case (E.eval_expr e m); simpl; trivial.
   Qed.
    
   Lemma cdeno_assert : forall e (mn:Mem.t k * nat),
    [[[ [Assert e] ]]] E mn ==
    if E.eval_expr e (fst mn) then Munit mn else distr0 _.
   Proof.
    intros e (m, n); apply eq_distr_intro; intro f.
    simpl fst.
    rewrite cdeno_assert_elim.
    simpl fst; case (E.eval_expr e m); trivial.
   Qed.

   Lemma deno_assert_elim : forall e m f,
    mu ([[ [Assert e] ]] E m) f == if E.eval_expr e m then f m else 0.
   Proof.
    intros; rewrite deno_base_elim.
    simpl; case (E.eval_expr e m); trivial.
   Qed.

   Lemma deno_assert : forall e m,
    [[ [Assert e] ]] E m == if E.eval_expr e m then Munit m else distr0 _.
   Proof.
    intros; apply eq_distr_intro; intro f.
 ...
 Proof. 
  ...
  destruct b.
  auto using is_Discrete_Munit, is_Discrete_Mlet.
  assert (H:=DE.ceval_support_spec s0 (st_mem s)); simpl in H; rewrite <-H.
  auto using is_Discrete_Munit, is_Discrete_Mlet, DE.discrete_support.
  destruct (E.eval_expr e (st_mem s)).
  apply is_Discrete_Munit.
  apply is_Discrete_distr0.
