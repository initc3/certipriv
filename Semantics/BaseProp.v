...
Module Type SEM_THEORY (Sem:SEM).
 ...
 Fixpoint fv_distr t (d:DE.support t) :=
  match d with
  | DE.Dbool => Vset.empty
  | DE.Dnat e => fv_expr e 
  | DE.DZ e1 e2 => Vset.union (fv_expr e1) (fv_expr e2)
  | DE.Dprod t1 t2 s1 s2  => Vset.union (fv_distr s1) (fv_distr s2) 
  | DE.Duser t s => US.fv_distr s
  end.
...
Module Make (S:SEM). (* <: SEM_THEORY S. *)
 ...
 Definition depend_only_distr (X:Vset.t) t (d:DE.support t) :=
  forall k (m1 m2:Mem.t k), 
   m1 =={X} m2 -> DE.eval_support d m1 = DE.eval_support d m2.

 Lemma depend_only_fv_distr : forall t (d:DE.support t), 
 ...
 Proof.
  ...
  red; intros.
  apply H; auto with set.
  apply (US.depend_only_fv_distr H).
  rewrite (IHd1 _ _ m2), (IHd2 _ _ m2); trivial; 
    try (eapply req_mem_weaken;[ | apply H]; auto with set).
 Qed.

 Lemma support_eq_fv_empty k t (s:DE.support t) (m1 m2:Mem.t k) :
  fv_distr s [=] Vset.empty ->
  DE.eval_support s m1 = DE.eval_support s m2.
 Proof.
  ...
  apply VsetP.empty_union in H; destruct H.
 ...
 Proof.
  ...
  transitivity (mu (DE.eval_support d m) (fun _ => 0)).
  symmetry; apply mu_0.
 ...
 Lemma Modify_assert : forall E e, 
  Modify E Vset.empty [Assert e].
 Proof.
  unfold Modify, range,eeq_mem; intros.
...
(** ** Observationnal equivalence of expressions *)
 ...
 (** Observational equivalence of a distribution expression *)

 Definition EqObs_d t X (d1 d2:DE.support t) := 
  forall k (m1 m2:Mem.t k), 
   m1 =={X} m2 -> DE.eval_support d1 m1 = DE.eval_support d2 m2.

 Lemma EqObs_d_strengthen : forall I1 I2,
  I1 [<=] I2 ->
  forall t (d d':DE.support t), EqObs_d I1 d d' -> EqObs_d I2 d d'.
 Proof.
  unfold EqObs_d; intros; eauto using req_mem_weaken.
 Qed.
 
 Definition Deq t (d1 d2:DE.support t) := d1 = d2.
 ...
 Add Parametric Relation t : (DE.support t) (Deq (t:=t)) 
 ...
 Lemma EqObs_d_fv_expr : forall t (d:DE.support t), EqObs_d (fv_distr d) d d.
 Proof depend_only_fv_distr.

 Hint Resolve EqObs_d_fv_expr.

 Lemma EqObs_d_refl : forall t (d:DE.support t) X, 
