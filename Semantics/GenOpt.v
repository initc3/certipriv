...
Module Make (SemO:SEM_OPT).
 ...
 Module MakeOpt (OE:OPT_ENTRIES).
  ...
  Fixpoint eval_d t (d:DE.support t) a :=  
   match d in (DE.support t0) return (DE.support t0) with 
   | DE.Dbool => DE.Dbool
   | DE.Dnat e  => DE.Dnat (eval_e e a)
   | DE.DZ e1 e2  => DE.DZ (eval_e e1 a) (eval_e e2 a)
   | DE.Duser t us => DE.Duser us
   | DE.Dprod t1 t2 s1 s2 => DE.Dprod (eval_d s1 a) (eval_d s2 a)
   end.
  ...
  Definition opt_bi (bi:I.baseInstr) (a:A.t) : cmd * A.t :=
   ...
   | I.Random t x d =>
     ...
     let a' := remove x a in
      ([ x <$- r ], a')
   | I.IAssert e =>
     let r := eval_be e a in
     match get_bool r with
     | Some true => ([Assert e], add_test e true a)
     | _ => ([Assert e], a)
     end
   end.
  ...
  Section OPT_I.
   ...
   Lemma valid_eval_d :  forall (a:A.t) k (m:Mem.t k) t (d:DE.support t),
    valid a m -> DE.eval_support d m = DE.eval_support (eval_d d a) m.
   Proof.
   ...
   Lemma is_true_bool_correct : forall k (m:Mem.t k) (e:E.expr T.Bool), 
    is_false_bool e -> E.eval_expr e m = false.
   Proof.
   ...
   Proof.
    ...
    (* Assert *)  
    case_eq (get_bool (eval_be e a)); intros.
    destruct b.
    eapply equiv_sub; [ | | eapply equiv_assert with (P:=valid_eq a)].
    unfold valid_eq; simpl; intros k m m' (H1,H2).
    rewrite <-H2; repeat split; trivial.
    intros k m1 m2 [ [H1 H2] [H3 _] ]; rewrite <-H3.
    split; [ | trivial].
    apply (get_bool_correct m1) in H.
    apply valid_add_test; trivial.  

    eapply equiv_sub; [ | | eapply equiv_assert with (P:=valid_eq a)].
    unfold valid_eq; simpl; intros k m m' (H1,H2).
    rewrite <-H2; repeat split; trivial.
    intros k m1 m2 [ [H1 H2] [H3 _] ]; split; trivial.
