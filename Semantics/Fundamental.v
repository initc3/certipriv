(** * Fundamental.v : Fundamental lemma of game-playing *)
 ...
 Section FUNDAMENTAL_LEMMA.
  ...
  Section PRESERVES_BAD.
   ...
   Proof.
    ...
    (* Assert *)
    repeat rewrite deno_assert_elim.
    case_eq (E.eval_expr e m); intro; [rewrite H0 | ]; trivial.
  ...
  Section UPTO.
   ...
   Proof.
    ...
    simpl in H; unfold is_true in H.
    apply andb_prop in H; destruct H as [Heq _].
    generalize (I.eqb_spec (v <- e) (Assert e0)); rewrite Heq; intro H.
    discriminate H.
    ...
    apply andb_prop in H; destruct H as [Heq Hc2].
    destruct c2; try discriminate.
    generalize (I.eqb_spec (v <$- s) (Assert e)).
    ...
    apply andb_prop in H; destruct H as [Heq Hc2].
    destruct c2; try discriminate.
    generalize (I.eqb_spec (Assert e) (v <- e0)).
    rewrite Heq; intro; discriminate.

    simpl in H; unfold is_true in H.
    apply andb_prop in H; destruct H as [Heq Hc2].
    destruct c2; try discriminate.
    generalize (I.eqb_spec (Assert e) (v <$- s)).
    rewrite Heq; intro; discriminate.

    simpl in H; unfold is_true in H.
    apply andb_prop in H; destruct H as [Heq Hc2].
    destruct c2; try discriminate.
    generalize (I.eqb_spec (Assert e) (Assert e0)).
    rewrite Heq; intro; injection H; intros; subst.
    repeat rewrite deno_assert_elim; trivial.
