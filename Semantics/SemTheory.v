...
Module Make (Sem:SEM).
 ...
 Lemma equiv_assert_l : forall P E1 E2 e,
  equiv (P /-\ EP1 e) E1 [Assert e] E2 nil P.
 Proof.
  unfold equiv; intros.
  exists (fun m1 m2 => Munit (m1, m2)).
  intros m1 m2 [H1 H2].
  rewrite deno_assert, deno_nil.
  rewrite H2.
  apply lift_unit; trivial.
 Qed.

 Lemma equiv_assert_r : forall P E1 E2 e,
  equiv (P /-\ EP2 e) E1 nil E2 [Assert e] P.
 Proof.
  unfold equiv; intros.
  exists (fun m1 m2 => Munit (m1, m2)).
  intros m1 m2 [H1 H2].
 ...
 Lemma equiv_assert : forall P E1 E2 e1 e2, 
  equiv 
  (P /-\ fun k m1 m2 => E.eval_expr e1 m1 = E.eval_expr e2 m2)
  E1 [Assert e1] E2 [Assert e2] 
  (P /-\ EP1 e1 /-\ EP2 e2).
 Proof.
  unfold equiv; intros.
  exists (fun m1 m2 => 
   if (E.eval_expr e1 m1 && E.eval_expr e2 m2)%bool then 
    Munit (m1, m2) else distr0 _).
  intros m1 m2 H.
  rewrite deno_assert, deno_assert.
  destruct H as [? H1]; rewrite <-H1.
  case_eq (E.eval_expr e1 m1); intro; simpl.
  apply lift_unit; repeat split; trivial.
  unfold EP2; rewrite <-H1; trivial.  
  apply distr0_lift; repeat split; trivial.
 Qed.

 Definition eq_support t (d1 d2:DE.support t) : mem_rel :=
  fun k (m1 m2:Mem.t k) => DE.eval_support d1 m1 = DE.eval_support d2 m2.
 ...
 Proof.
  ...
  generalize (T.i_eqb_spec k t x x0);  case (T.i_eqb k t x x0); intuition.  
 Qed.
 ...
 Definition forall_random (Q:mem_rel) t (x1 x2:Var.var t) d : mem_rel :=
  fun k m1 m2 => 
   forall i,
    0 < coeff (carT k t) 
    (D_points (DE.discrete_support d m1)) (DE.eval_support d m1) i ->
    let v := D_points (DE.discrete_support d m1) i in
     Q k (m1{!x1 <-- v!}) (m2{!x2 <-- v!}).

 Lemma equiv_random : forall (Q:mem_rel) E1 E2 t (x1 x2:Var.var t) 
  (d1 d2:DE.support t),
 ...
 Proof.
  unfold equiv; intros.
  exists (fun m1 m2 => 
   Mlet (DE.eval_support d1 m1) (fun v => Munit (m1{!x1<--v!}, m2{!x2<--v!}))).
  ...
  intros f Hf; simpl.
  rewrite (mu_is_Discrete _ (carT_cover _ _) (DE.discrete_support d1 m1)).
  symmetry; simpl; apply serie_zero; intro i.
  set (c:=coeff (carT k t) (D_points (DE.discrete_support d1 m1)) 
   (DE.eval_support d1 m1) i). 
  apply (Ueq_orc c 0); [apply Ueq_class | | ]; intro.
  rewrite H; auto.
  rewrite <-Hf; [Usimpl; trivial | ].
  red; simpl; auto.
 Qed.

 Definition permut_support (A B:Type) (d1:Distr A) (d2:Distr B)
  (Q:A -> B -> Prop) (f:A -> B) :=
  (forall h, mu d2 h == mu (Mlet d1 (fun a => Munit (f a))) h) /\ 
  forall a, Q a (f a).

 Lemma equiv_random_permut : forall (Q:mem_rel) E1 E2 t (x1 x2:Var.var t) 
  (f:forall k (m1 m2:Mem.t k), T.interp k t -> T.interp k t)
  (d1 d2:DE.support t),
  let Q' k m1 m2 := 
   fun (a b:T.interp k t) => Q k (m1{!x1 <-- a!}) (m2{!x2 <-- b!}) in
  equiv (fun k (m1 m2:Mem.t k) => 
   permut_support (DE.eval_support d1 m1) (DE.eval_support d2 m2) (Q' k m1 m2) 
   (f k m1 m2))
  E1 [x1 <$- d1] E2 [x2 <$- d2] Q.
 Proof.
  unfold equiv; intros.
  exists (fun m1 m2 =>
   Mlet (DE.eval_support d1 m1)
   (fun a => Munit (m1{!x1 <-- a!}, m2{!x2 <-- f k m1 m2 a!}))).
  intros.
  unfold permut_support in H; decompose [and] H; clear H.
  constructor; intros.
...
Qed.
 ...
 Lemma equiv_strengthen_range: forall E c1 c2 (P Q:mem_rel) 
  (R:forall k, Mem.t k -o> boolO),
  decMR Q ->
  (forall k (m1 m2:Mem.t k), P k m1 m2 -> 
    range (R k) (([[c1]]) E m1)) ->
  equiv P E c1 E c2 Q ->
  equiv P E c1 E c2 (Q /-\ (fun k (m1 _:Mem.t k) => R _ m1)).
 Proof.
  unfold equiv; intros.
  destruct (H0 k) as [d Hd]; clear H0.
  exists d; intros; constructor.
  apply (l_fst (Hd m1 m2 H0)).
  apply (l_snd (Hd m1 m2 H0)).
  apply range_weaken with 
   (P1:=fun v => 
    ((fun xy => if @X k (fst xy) (snd xy) then true else false) [&&] 
     (fun xy => R k (fst xy))) v = true).
  unfold prodP, andP; intros; rewrite andb_true_iff in H1; destruct H1.
  split. 
  destruct (X k (fst x) (snd x)); [trivial | discriminate].
  trivial.
  apply range_strengthen.
  apply range_weaken with (2:=l_range (Hd m1 m2 H0)).
  intros (m1',m2'); unfold prodP, fst, snd.
  destruct (X k m1' m2'); [trivial | intro; tauto].
  intros f Hf.
  rewrite (mu_restr_split (d m1 m2) (fun mm => R k (fst mm))).
  rewrite <-Uplus_zero_right; apply Uplus_eq_compat.
    rewrite <-(mu_zero (d m1 m2)); apply mu_stable_eq; 
      refine (ford_eq_intro _); intro mm.
    generalize (Hf mm); unfold fzero, restr; case (R k (fst mm)); auto. 
    split; trivial.
    transitivity (mu (d m1 m2) 
      (charfun (negP (fun mm : Mem.t k * Mem.t k => R k (fst mm))))).
      apply mu_monotonic; apply restr_le_compat; auto.
    unfold charfun, restr, fone, negP, negb.
    rewrite (l_fst (Hd _ _ H0) (fun m: Mem.t k =>
      if if R k m then false else true then 1 else 0)).
    apply Oeq_le; symmetry; apply (H _ _ _ H0).
    intros m Hm; rewrite Hm; trivial.
 Qed.
 ...
 Lemma EqObs_random : forall E1 E2 t (v:Var.var t) (d:DE.support t),
 ...
 Lemma EqObs_Pr2 : forall I E1 c1 E2 c2 O,
  ...
  forall k (m1 m2:Mem.t k), m1 =={I} m2 -> 
   Pr E1 c1 m1 (EP k e) == Pr E2 c2 m2 (EP k e).
 Proof.
  unfold Pr; intros.
